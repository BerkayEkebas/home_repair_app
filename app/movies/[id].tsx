      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        <View className='flex-col items-start justify-center mt-5 px-5'>
          ...
          <View className='flex flex-row justify-between w-1/2'>
            <MovieInfo
              label='Budget'
              value={movie?.budget != null ? `$${(movie.budget / 1_000_000).toFixed(1)} million` : 'N/A'}
            />
            <MovieInfo
              label='Revenue'
              value={movie?.revenue != null ? `$${(movie.revenue / 1_000_000).toFixed(1)} million` : 'N/A'}
            />

          </View>
          <MovieInfo label='Production Compianies' value={movie?.production_companies.map((c) => c.name).join(' - ') || 'N/A'} />
        </View>
      </ScrollView>
